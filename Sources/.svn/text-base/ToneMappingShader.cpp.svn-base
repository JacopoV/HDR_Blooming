...
bool ToneMappingShader::Initalize(ID3D11Device* pd3dDevice)
{
	...
	HRESULT hr = D3DX11CompileFromFile(L"Shaders/tonemapping.fx", NULL, NULL,NULL, "fx_5_0", dwShaderFlags, 0,
		NULL, &pBlob, &pErrorBlob, NULL);


    if( FAILED( hr ) )
    {
		MessageBox(NULL, L"Problema nel caricamento di tonemapping.fx.", L"Error", MB_OK );
	...
	m_Technique = m_Effect->GetTechniqueByName( "ToneMapping" );
...
void ToneMappingShader::SetShaderTexturePhong(ID3D11ShaderResourceView* texture)
{
	m_Effect->GetVariableByName("g_texture_phong")->AsShaderResource()->SetResource(texture);
}
